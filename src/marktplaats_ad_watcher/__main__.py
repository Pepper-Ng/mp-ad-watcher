...
def main() -> None:
    ...
    mode.add_argument(
        "--repair-budget-retry-state",
        action="store_true",
        help="Repair ads that were incorrectly exhausted by local model-budget limits.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.repair_budget_retry_state:
        store = build_watcher(
            settings,
            status_store=RuntimeStatusStore(settings.status_file),
        )._store
        repaired, backup_path = store.repair_budget_retry_exhaustion()
        print(
            {
                "repaired_ads": repaired,
