...
def create_web_app(*, env_file: Path, dry_run: bool = False) -> Starlette:
    ...
    async def config_get(request: Request) -> HTMLResponse:
        ...
        notify_ai_failure_checkbox = _checkbox(
            "NOTIFY_AI_FAILURES", values, "Send AI failure alerts to Telegram"
        )
                ...
                <form class="config-form" method="post" action="/config{token_query}">
                    ...
                    <fieldset>
                        ...
                        <details class="advanced">
                            ...
                            <p class="hint">Use this only when the primary model fails.
                            JSON mode remains explicitly configurable because support varies by
                            provider and model.</p>
                        </details>
